# --------------------------------------------------------------- #
# plot group level functions, exp. 2
...
group_dat <- {}
for(id_i in ids){
  file_saccade <- paste("./results/xp2/",id_i,"_saccade_xp2_fit.RDS",sep="")
  file_perception <- paste("./results/xp2/",id_i,"_perception_xp2_fit.RDS",sep="")
...
for(i in 2:length(group_dat$signi)){
  ...
  if(!is.na(group_dat$signi[i-1]) & is.na(group_dat$signi[i]) & inblob==1){
    ...
    d_line <- data.frame(n=n_blob, s=blob_length, type=group_dat$type[i-1],p = blob_p, blob_start=start_blob, blob_end = end_blob, blob_start_D= start_blob_D, blob_end_D = end_blob_D, id=group_dat$id[i])
...
ggsave("./figs/group_xp2.pdf", plot=group_plot, device="pdf", width=6,height=2.5)
